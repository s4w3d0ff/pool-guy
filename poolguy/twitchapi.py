...
class TwitchApi(RequestHandler):
    ...
    async def getChannelFollowers(self, broadcaster_id=None, first=None):
        ...
        out = r['data']
        page = r.get('pagination') or {}
        if 'cursor' in page:
            out += await self._continuePage(method, url, page, params=params)
        return out
    ...
    async def getStreams(self, first=None, **kwargs):
        ...
        page = r.get('pagination') or {}
        if 'cursor' in page:
            out += await self._continuePage("get", self.apiEndpoints['streams'], page, params=params)
        return out
